...
def draw_feature_map(features,save_dir = 'feature_map',img_metas = None,i=0):
    ...
    if isinstance(features,torch.Tensor):
        ...
        for featuremap in features:
            ...
            for j, heatmap in enumerate(heatmaps):
                img = mmcv.imread(img_metas[j]['filename'])
                ...
                # cv2.waitKey(0)
                save_dir1= os.path.join(save_dir,img_metas[j]['ori_filename'][:-4])
                if not os.path.exists(save_dir1):
                    os.makedirs(save_dir1)
                #print(os.path.join(save_dir1,str(i)+'.png'))
                cv2.imwrite(os.path.join(save_dir1,str(i)+'.png'), superimposed_img)
