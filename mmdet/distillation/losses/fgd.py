...
class FeatureLoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.model = DiffusionModel(channels_in=teacher_channels, kernel_size=3)
        self.scheduler = DDIMScheduler(num_train_timesteps=1000, clip_sample=False, beta_schedule="linear")
        self.noise_adapter = NoiseAdapter(teacher_channels, 3)
...
#
    ...
    def forward(self,
                ...
                gt_bboxes,
                img_metas,
                scale_y=None, scale_x=None,yolox=False
                ):
        """Forward function.
        Args:
        ...
        refined_preds_S = self.pipeline(
                                    batch_size=N,
                                    device=preds_S.device,
                                    dtype=preds_S.dtype,
                                    shape=preds_S.shape[1:],
                                    feat=preds_S,
        ...
        if yolox: 
            ...
            S_attention_s, C_attention_s = self.get_attention(refined_preds_S, self.temp)
            ...
            fg_loss, bg_loss = self.get_fea_loss_yolox(refined_preds_S, preds_T, Mask_fg, Mask_bg, 
            ...
            rela_loss = self.get_rela_loss(refined_preds_S, preds_T)

            loss1 = 0.002 * fg_loss + 0.001 * bg_loss \
                + 0.001 * mask_loss + 0.00001 * rela_loss
            
            fea_loss, ddim_loss = self.get_fea_loss(refined_preds_S, preds_T)
            
            loss2 = self.alpha_fgd * fea_loss + self.beta_fgd * ddim_loss

            return loss1/scale_x/scale_y+loss2
        else:
            fea_loss, ddim_loss = self.get_fea_loss(refined_preds_S, preds_T)

            loss = self.alpha_fgd * fea_loss + self.beta_fgd * ddim_loss
    ...
    def get_fea_loss_yolox(self, preds_S, preds_T, Mask_fg, Mask_bg, C_s, C_t, S_s, S_t):
    ...
    def get_fea_loss(self, preds_S, preds_T):
        N, C, H, W = preds_S.shape
        ...
        #    proj=self.proj
        #)
        refined_feat = self.proj(preds_S)

        refined_feat = refined_feat.view(N, C, -1)
        refined_feat = F.softmax(refined_feat, dim=2)
        target = preds_T.view(N, C, -1)
        target = F.softmax(target, dim=2)

        fea_loss = F.kl_div(torch.log(refined_feat), target, reduction='batchmean')

        # train diffusion model
        ddim_loss = self.ddim_loss(preds_T) / N

        return fea_loss, ddim_loss
