...
class DetectionDistiller(BaseDetector):
    ...
    def forward_train(self, img, img_metas, **kwargs):
        ...
        if self.yolox:
            ...
            student_loss, img, gt,s_input_size = self.student.forward_train(img, img_metas, **kwargs)
        else:
            student_loss = self.student.forward_train(img, img_metas,is_student=True, **kwargs)   
        
        with torch.no_grad():
           
            #feat = self.teacher.extract_feat(img)
            if self.yolox:
                feat,_,_,_=self.teacher.forward_train(img, img_metas,is_student=False,input_size=s_input_size, **kwargs)
            else:
                feat = self.teacher.extract_feat(img)  
        ...
        for item_loc in self.distill_cfg:
            ...
            for item_loss in item_loc.methods:
                ...
                if self.yolox:
                    student_loss[loss_name] = self.distill_losses[loss_name](student_feat, teacher_feat, gt,img_metas,scale_y, scale_x,True)
                else:
                    student_loss[loss_name] = self.distill_losses[loss_name](student_feat,teacher_feat,kwargs['gt_bboxes'],
                                                                         img_metas)
